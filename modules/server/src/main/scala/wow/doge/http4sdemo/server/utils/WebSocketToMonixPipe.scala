...
class WebSocketToMonixPipe[_F[
    _
]: ConcurrentEffect, R <: MonixStreams with WebSockets]
    extends WebSocketToPipe[R] {
  override type S = MonixStreams
  override type F[X] = _F[X]

  override def apply[REQ, RESP](s: Any)(
      ws: WebSocket[F],
      o: WebSocketBodyOutput[Any, REQ, RESP, _, MonixStreams]
  ): Any = { in: Observable[REQ] =>
    val sends = in
      .map(o.requests.encode)
      .mapEvalF(
        ws.send(_, isContinuation = false)
      ) // TODO support fragmented frames
    ...
    def decode(frame: WebSocketFrame) =
      o.responses.decode(frame) match {
        ...
        case DecodeResult.Value(v) =>
          (Right(Some(v)): Either[Unit, Option[RESP]]).pure[F]
      }
    ...
    val receives = fs2.Stream
      ...
      .evalMapAccumulate[F, Option[WebSocketFrame], Either[Unit, Option[RESP]]](
        ...
        case (prev, frame @ WebSocketFrame.Text(_, last, _)) =>
          concatOrDecode(prev, frame, last)((l, r) =>
            r.copy(payload = l.payload + r.payload)
          )
    ...
    Observable(
      sends,
      Observable.fromReactivePublisher(receives.toUnicastPublisher)
    ).merge
  }
}
