          ...
          _ <- Task.parSequence(
            List(
              ...
              observableToJsonStreamA(
                Observable.repeatEvalF(queue.poll).take(10)
              ).flatMap(o =>
                basicRequest
                  .post(uri"http://localhost:8081/api/publish2")
                  .streamBody(Fs2Streams[Task])(o)
              ...
              basicRequest
                ...
                .response(
                  asStream(Fs2Streams[Task])(byteStream =>
                    for {
                      obs <- observableFromByteStreamA[StreamEvent](byteStream)
                      _ <- obs
                        .take(10)
                        .doOnNext(event => queue.offer(event).toTask)
                        .completedL
...
@main def run() = program.use(_ => Task.sleep(10.seconds)).runSyncUnsafe()
